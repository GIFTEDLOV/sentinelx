...
import { createTransactionSnapshot, getServerTransactionSnapshot, subscribeTransactions } from "@/lib/genlayer/transactions";
import { shortHash } from "@/lib/workflow";
import { Card, EmptyState, PageHeader, StatusBadge } from "./ui";

export function ActivityPage() {
  const [filter, setFilter] = useState("");
  const snapshot = useMemo(() => createTransactionSnapshot(), []);
  const records = useSyncExternalStore(subscribeTransactions, snapshot, getServerTransactionSnapshot);
