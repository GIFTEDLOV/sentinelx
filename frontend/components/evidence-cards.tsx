...
export function EvidenceCards({ proposal, policy, envelopes, snapshot, reviewWebFetchCount }: { proposal: ReleaseProposal; policy?: TargetPolicy; envelopes?: Partial<Record<"source" | "ci" | "security", EvidenceEnvelope>>; snapshot?: EvidenceSnapshot; reviewWebFetchCount?: number }) {
  ...
  return <div className="page-grid"><div className="notice info"><strong>Semantic review executes from authenticated stored evidence.</strong><div>Review-time web fetches: {typeof reviewWebFetchCount === "number" ? reviewWebFetchCount : captured ? 0 : "not proven until capture"}.</div></div><div className="evidence-grid">
