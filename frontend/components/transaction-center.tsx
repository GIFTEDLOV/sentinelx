...
import { getServerTransactionSnapshot, listTransactions, subscribeTransactions } from "@/lib/genlayer/transactions";
import { shortHash } from "@/lib/workflow";
import { StatusBadge } from "./ui";

export function TransactionCenter({ open, onClose }: { open: boolean; onClose: () => void }) {
  const records = useSyncExternalStore(subscribeTransactions, listTransactions, getServerTransactionSnapshot);
  if (!open) return null;
